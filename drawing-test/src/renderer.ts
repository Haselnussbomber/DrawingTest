...
const MyRenderer = Reconciler({
  ...
  createInstance(
    ...
    //console.debug(`createInstance called with type: ${type} => ${instance.DisplayName}`);
    for (const name in props) {
      instance.SetProp(name, props[name]);
    }
    return instance;
  },
  ...
  createTextInstance(
    ...
    const instance = document.CreateTextElement();
    instance.TextValue = text;
    console.debug(`createTextInstance called with text: ${text} => ${instance.DisplayName}`);
    return instance;
  },
  ...
  appendInitialChild(parentInstance: Instance, child: Instance | TextInstance): void {
    console.debug(`appendInitialChild called with parentInstance: ${parentInstance.DisplayName}, child: ${child.DisplayName}`);
  ...
  finalizeInitialChildren(
    ...
    console.debug(`finalizeInitialChildren called for ${type}`);
    return false;
  },
  ...
  prepareUpdate(
    ...
    for (const propName in oldProps) {
      ...
      if (typeof(newPropValue) == 'undefined') {
        map.set(propName, undefined);
        // console.trace(`${propName} removed`);
      } else if (oldPropValue != newPropValue) {
        map.set(propName, newPropValue);
        // console.trace(`${propName} changed: ${oldPropValue} -> ${newPropValue}`);
      }
    }
    ...
    for (const propName in newProps) {
      ...
      if (typeof(oldPropValue) == 'undefined') {
        // console.trace(`${propName} added`);
        map.set(propName, newPropValue);
      }
    }
  ...
  appendChild(parentInstance: Instance, child: Instance | TextInstance): void {
    console.debug(`appendChild called with parentInstance: ${parentInstance.DisplayName}, child: ${child.DisplayName}`);
    parentInstance.Add(child);
  },
  ...
  appendChildToContainer(container: Container, child: Instance | TextInstance): void {
    console.debug(`appendChildToContainer called with container: ${container.DisplayName}, child: ${child.DisplayName}`);
    container.Add(child);
  },
  ...
  insertBefore(
    ...
    const index = parentInstance.IndexOf(beforeChild);
    parentInstance.Insert(index, child);
  },
  ...
  commitUpdate(
    ...
    // console.debug(`commitUpdate called on ${instance.DisplayName} with updatePayload: ${JSON.stringify(updatePayload.size)}`);
    updatePayload.forEach((value, key) => {
      if (value != undefined)
        instance.SetProp(key, value);
      else
        instance.UnsetProp(key);
    });
  },
...
export function render(element: ReactNode, container: any, callback?: (() => void) | null) {
