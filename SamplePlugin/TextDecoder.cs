...
public unsafe class TextDecoder(IPluginLog Logger, IDataManager DataManager)
{
    private readonly Dictionary<(ClientLanguage Language, string SheetName, int RowId, int Amount, int Person, int Case), string> Cache = [];
    ...
    public string ProcessNoun(ClientLanguage language, string SheetName, int Person, int RowId, int Amount = 1, int Case = 1, int UnkInt5 = 1)
    {
        ...
        if (row == null)
        {
            Logger.Warning("Sheet {SheetName} does not contain row #{RowId}", SheetName, RowId);
            return string.Empty;
        }
    ...
    private Utf8String* ResolveNounDe(int Amount, int Person, int Case, RawExcelSheet attributiveSheet, RowParser row, int columnOffset)
    {
        /*
             a1->Offsets[0] = SingularColumnIdx
             a1->Offsets[1] = PluralColumnIdx
